 ...
 */
class TimerRoutineEditViewModel(
    ...
    private val timerRoutineId: Int = checkNotNull(savedStateHandle[TimerRoutineEditDestination.timerRoutineIdArg])

    init {
        viewModelScope.launch {
            timerRoutineUiState = routinesRepository.getTimerRoutineStream(timerRoutineId)
                .filterNotNull()
                .first()
                .toTimerRoutineUiState(true)
        }
    }
     ...
     */
    suspend fun updateTimerRoutine() {
        if (validateInput(timerRoutineUiState.timerRoutineDetails)) {
            routinesRepository.updateTimerRoutine(timerRoutineUiState.timerRoutineDetails.toTimerRoutine())
        }
    }
